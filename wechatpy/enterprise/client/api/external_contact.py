...
class WeChatExternalContact(BaseWeChatAPI):
    ...
    def list_groupchat(
        self, status_filter=0, owner_filter=None, cursor=None, limit=1000
    ):
        """
        获取客户群列表
        https://work.weixin.qq.com/api/doc/90001/90143/93414
        :return: 返回的 JSON 数据包
        """
        data = optionaldict(
    ...
    def iter_groupchat(
        self, status_filter=0, owner_filter=None, cursor=None
    ):
        """
        获取客户群列表
        https://work.weixin.qq.com/api/doc/90001/90143/93414
        ...
        """
        while True:
            data = self.list_groupchat(status_filter, owner_filter, cursor)
            for group_chat in data['group_chat_list']:
                yield group_chat
            cursor = data.get('next_cursor')
            if not cursor:
                return

    def get_groupchat(self, chat_id):
        """
        获取客户群列表
        https://work.weixin.qq.com/api/doc/90001/90143/92707
        :return: 返回的 JSON 数据包
        """
