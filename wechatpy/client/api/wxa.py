...
class WeChatWxa(BaseWeChatAPI):
    ...
    def generate_url_link(
        ...
        """
        获取加密URLLink，获取小程序 URL Link，适用于短信、邮件、网页、微信内等拉起小程序的业务场景。
        详情请参考
        ...
        :param expire_type: 默认值0.小程序 URL Link 失效类型，失效时间：0，失效间隔天数：1
        :param expire_time: 到期失效的 URL Link 的失效时间，为 Unix 时间戳。生成的到期失效 URL Link 在该时间前有效。最长有效期为30天。expire_type 为 0 必填
    ...
    def query_url_link(self, url_link, query_type=0):
        """
        查询加密URLLink，该接口用于查询小程序加密 url_link 配置
        详情请参考
        https://developers.weixin.qq.com/miniprogram/dev/OpenApiDoc/qrcode-link/url-link/queryUrlLink.html

        :param url_link: 小程序 url_link
        :param query_type: 查询类型。默认值0
        :return:
        """
        return self._post(
            "wxa/query_urllink",
